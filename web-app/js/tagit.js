    ...
    $.widget("ui.tagit", {
        ...
        _addTag: function(label, value) {
            ...
            tag = $('<li class="tagit-choice '
                + (this._existsInitially(label, value) ? 'exists' : 'new-tag') + '"'
        ...
        _existsInitially: function(label, value) {
            if (this.options.tagSource.length == 0)
               return false;
            
            for(var ind in this.options.tagSource) {
               var _label = this.options.tagSource[ind];
